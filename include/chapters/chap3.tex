...
Iniziamo considerando l'esecuzione dell'algoritmo di Frank-Wolfe su istanze di dimensioni differenti, relativamente al numero
di elementi della matrice di riferimento, al variare del valore di sparsità. I risultati sono riassunti nei grafici
della Figura \ref{fig:bysize}.
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{assets/figures/bysize.pdf}
...
\noindent
Osserviamo che quando la sparsità della matrice di riferimento è sufficientemente elevata, l'algoritmo di Frank-Wolfe
riesce a chiudere sul valore ottimo con entrambe le limitazioni, indipendentemente dalla dimensione dell'istanza.
Per
valori di sparsità inferiori, la situazione è molto diversa e il numero degli elementi della matrice di riferimento
influenza significativamente la qualità della limitazione superiore, ma non quella della limitazione inferiore. In
particolare, a parità di sparsità, l'aumentare del numero di elementi determina un peggioramento nella qualità della
limitazione superiore prodotta dall'algoritmo di Frank-Wolfe. La limitazione inferiore ha invece un andamento più
irregolare.
Per matrici che hanno un numero ridotto di elementi, l'aumento della sparsità è indicatore di un miglioramento della
limitazione inferiore. Al contrario, quando la dimensione delle istanze cresce, l'aumento della sparsità non corrisponde
più a un miglioramento nella qualità della limitazione inferiore. Inoltre, per istanze di dimensione sufficientemente
elevata, la variazione di sparsità non è così influente sulla qualità della limitazione inferiore, che è molto più
stabile rispetto a quella della limitazione superiore. In generale, la qualità della limitazione inferiore rimane buona
per tutte le dimensioni che abbiamo utilizzato negli esperimenti.

\subsection{Forma della matrice di riferimento}
Consideriamo ora istanze caratterizzate da matrici di riferimento che hanno lo stesso numero di elementi ma forma
diversa, per capire come quest'ultima influenza la qualità delle limitazioni prodotte dall'algoritmo di Frank-Wolfe,
al variare del valore di sparsità.
I risultati dell'esperimento sono riassunti dai grafici della Figura
\ref{fig:byshape_small}, in cui ciascuna riga contiene grafici relativi a istanze caratterizzate da matrici di
riferimento che hanno lo stesso numero di elementi.

Notiamo che la forma della matrice di riferimento influenza la qualità delle limitazioni prodotte dall'esecuzione
dell'algoritmo. In particolare, la limitazione superiore peggiora all'aumentare del numero di variabili di un'istanza,
quando la sparsità è il numero degli elementi rimane costante, e migliora, per una dimensione fissata della matrice di
riferimento,
all'aumentare della sparsità.
L'andamento della limitazione inferiore è invece più irregolare. Ad esempio, per le
istanze caratterizzate da matrici \( 100\times 100 \) e \( 200\times 50 \) i risultati sperimentali evidenziano che
per valori di sparsità più bassi, le limitazioni inferiori sono migliori per le istanze con un numero maggiore di
variabili. Quando invece i valori di sparsità sono più elevati, sono le istanze caratterizzate dal minor numero di variabili
ad ottenere le limitazioni inferiori migliori. Lo stesso vale per le istanze con matrici \( 100\times 50
\) e  \( 200\times 25 \), ma non per le istanze di dimensione maggiore, caratterizzate da matrici \( 1000\times 1000 \)
e  \( 2000\times 500 \). Per queste ultime, le limitazioni inferiori migliori sono sempre quelle delle istanze in cui il
numero delle variabili è maggiore.
\begin{figure}[!ht]
    \centering
    \includegraphics[width=\textwidth]{assets/figures/byshape.pdf}
    \caption{qualità delle limitazioni prodotte dall'algoritmo di Frank-Wolfe per istanze con matrici di riferimento
    caratterizzate dallo stesso numero di elementi, al variare della sparsità e del numero di variabili.}
    \label{fig:byshape_small}
\end{figure}

Infine, consideriamo le istanze che hanno lo stesso numero di variabili ma un numero differente di vincoli. I risultati
ottenuti sono quelli della Figura \ref{fig:byshape2}, in cui ciascuna riga rappresenta istanze con lo stesso numero di
variabili.

Osserviamo che, a parità di sparsità, l'aumento nel numero dei vincoli determina un peggioramento della
limitazione superiore. Ancora una volta, l'andamento della limitazione inferiore è irregolare al variare della sparsità.

Se confrontiamo i risultati di questo esperimento con quelli ottenuti in precedenza, notiamo che la qualità della
limitazione inferiore non dipende tanto dal numero di variabili o dal numero di vincoli dell'istanza, ma quanto più dal
bilanciamento di questi parametri. Quando il numero delle righe e delle colonne è bilanciato, l'algoritmo di Frank-Wolfe
individua limitazioni inferiori migliori rispetto a situazioni in cui
ci sono molti più vincoli che variabili. Ad esempio, per matrici \( 100\times 100 \) il valore della limitazione
inferiore è mediamente migliore, a parità di sparsità, di quello ottenuto per matrici  \( 500\times 100 \), che a sua volta è migliore del
valore ottenuto per matrici  \( 1000\times 100 \).
\begin{figure}[!ht]
    \centering
    \includegraphics[width=\textwidth]{assets/figures/byshape_2.pdf}
    \caption{qualità delle limitazioni prodotte dall'algoritmo di Frank-Wolfe per istanze con lo stesso numero di
    variabili, al variare della sparsità e del numero di vincoli.}
    \label{fig:byshape2}
\end{figure}
