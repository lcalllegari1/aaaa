...
\begin{itemize}
    ...
    \item \text{\alt Passo 1}: trovare \( \vec{d} \in \mathcal{D} \) che massimizza l'approssimazione lineare di \( f
        \) in \( \vec{x}_k \), definita dal subgradiente \( \vec{s}_k \) di \( f \) in \( \vec{x}_k \). Formalmente,
        dobbiamo calcolare
        \[
...
\begin{algorithm}[H]
    ...
    \While{true}{\vspace*{10pt}
        ...
        $\vec{x}^{k+1} = (1 - \gamma)\vec{x}^k + \gamma \vec{d}$ with $\gamma = \frac{2}{k+2}$ \hfill {\alt {\ttfamily
    /*}  Passo 2 {\ttfamily */}}}
\end{algorithm}
\end{tcolorbox}
\noindent
